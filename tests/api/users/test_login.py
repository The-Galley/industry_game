...
API_URL = URL("/api/v1/users/login/")
...
async def test_login_unauthorized_error(api_client: TestClient):
...
async def test_login_successful_status_ok(
...
@pytest.mark.parametrize("user_type", (UserType.PLAYER, UserType.ADMIN))
async def test_login_successful_format(
...
@pytest.mark.parametrize("user_type", (UserType.PLAYER, UserType.ADMIN))
async def test_login_successful_set_cookie(
    api_client: TestClient,
    token_from_user,
    create_user,
    user_type,
